...
/** Draw 4 markers for mobile scanner to find the grid */
function drawAnchors(doc: jsPDF, x: number, y: number, w: number, h: number): void {
  const s = 1.8; // marker size
  doc.setFillColor(0, 0, 0);
  doc.rect(x - s, y - s, s, s, 'F'); // TL
  doc.rect(x + w, y - s, s, s, 'F'); // TR
  doc.rect(x - s, y + h, s, s, 'F'); // BL
  doc.rect(x + w, y + h, s, s, 'F'); // BR
}
...
async function drawBlank(
  ...
  // Add scanning anchors around the grid card
  drawAnchors(doc, ox + 3, cy, w - 6, gridH + CARD_PADDING * 2);
